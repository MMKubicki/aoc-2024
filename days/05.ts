...
export default async (fileRoot: string) => {
  ...
  const insertIntoRulemap = (
    ruleMap: Map<number, Array<[number, number]>>,
    at: number,
    rule: [number, number],
  ): void => {
  ...
  // sum up middles
  const part1Sum = orderedPages.map(getMiddle).reduce(
    (acc, curr) => acc + curr,
    0,
  );
  ...
  const part2Sum = unorderedPages.map((p) => orderPage(rules, p)).map(getMiddle)
