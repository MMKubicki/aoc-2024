...
export default async (fileRoot: string) => {
  ...
  for (const [y, row] of field.entries()) {
    for (const [x, char] of row.entries()) {
      ...
      if (
        x + 3 < xLength &&
        [char, field[y][x + 1], field[y][x + 2], field[y][x + 3]].join("") ===
          "XMAS"
      ) {
        foundCount += 1;
      }
      ...
      if (
        y + 3 < yLength &&
        [char, field[y + 1][x], field[y + 2][x], field[y + 3][x]].join("") ===
          "XMAS"
      ) {
        foundCount += 1;
      }
      ...
      // diagonal down right
      if (
        x + 3 < xLength && y + 3 < yLength &&
        [char, field[y + 1][x + 1], field[y + 2][x + 2], field[y + 3][x + 3]]
            .join("") === "XMAS"
      ) {
        foundCount += 1;
      }
